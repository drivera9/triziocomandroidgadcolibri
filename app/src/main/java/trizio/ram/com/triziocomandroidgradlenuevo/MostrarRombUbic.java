        ...
        if (conexion.equals("local")) {
            ...
            if ((atributo.trim().equals("PROMESA"))){
                ...
                if (atributo.trim().equals("CL")) {
                    ...
                    if (atributo.trim().equals("modificarMarca")) {
                        List<NameValuePair> params = new ArrayList<NameValuePair>();
                        url = "http://" + ip + "/consultarRecursoGad.php";
                        params.add(new BasicNameValuePair("sParametro", "modificarMarca"));
                        params.add(new BasicNameValuePair("sCodEmpresa", cod_empresa.trim()));
                        params.add(new BasicNameValuePair("sCodSucursal", cod_sucursal.trim()));
                        String resultServer = getHttpPost(url, params);
                        System.out.println("---------------------------------");
                        ...
                        try {
                            ...
                            for (int i = 0; i < jArray.length(); i++) {
                                JSONObject json = jArray.getJSONObject(i);
                                procesos[i] = json.getString("descripcion");
                                ids[i] = json.getString("marca");
                            }
                            ...
                            gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                ...
                                public void onItemClick(AdapterView<?> parent, View v,
                                                        int position, long id) {
                                    Intent data = new Intent();
                                    data.putExtra("parametro", "modificarMarca");
                                    data.putExtra("valor", procesos[position].trim());
                                    data.putExtra("id", ids[position].trim());
                                    setResult(RESULT_OK, data);
                                    finish();
                                }
                            });
                        ...
                        if (atributo.trim().equals("modificarReferencia")) {
                            List<NameValuePair> params = new ArrayList<NameValuePair>();
                            url = "http://" + ip + "/consultarRecursoGad.php";
                            params.add(new BasicNameValuePair("sParametro", "modificarReferencia"));
                            params.add(new BasicNameValuePair("sCodEmpresa", cod_empresa.trim()));
                            params.add(new BasicNameValuePair("sCodSucursal", cod_sucursal.trim()));
                            String resultServer = getHttpPost(url, params);
                            System.out.println("---------------------------------");
                            ...
                            try {
                                ...
                                gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                    ...
                                    public void onItemClick(AdapterView<?> parent, View v,
                                                            int position, long id) {
                                        Intent data = new Intent();
                                        data.putExtra("parametro", "modificarReferencia");
                                        data.putExtra("valor", procesos[position].trim());
                                        data.putExtra("id", ids[position].trim());
                                        setResult(RESULT_OK, data);
                                        finish();
                            ...
                            if (atributo.trim().equals("modificarAno")) {
                                List<NameValuePair> params = new ArrayList<NameValuePair>();
                                url = "http://" + ip + "/consultarRecursoGad.php";
                                params.add(new BasicNameValuePair("sParametro", "modificarAno"));
                                params.add(new BasicNameValuePair("sCodEmpresa", cod_empresa.trim()));
                                params.add(new BasicNameValuePair("sCodSucursal", cod_sucursal.trim()));
                                String resultServer = getHttpPost(url, params);
                                System.out.println("---------------------------------");
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < jArray.length(); i++) {
                                        JSONObject json = jArray.getJSONObject(i);
                                        procesos[i] = json.getString("modelo") + "-" + json.getString("ano");
                                        ids[i] = json.getString("id_modano");
                                    }
                                    ...
                                    gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                        ...
                                        public void onItemClick(AdapterView<?> parent, View v,
                                                                int position, long id) {
                                            Intent data = new Intent();
                                            data.putExtra("parametro", "modificarAno");
                                            data.putExtra("valor", procesos[position].trim());
                                            data.putExtra("id", ids[position].trim());
                                            setResult(RESULT_OK, data);
                                            finish();
                                ...
                                if (atributo.trim().equals("RAZON2")) {
                                    ...
                                    if (atributo.trim().equals("RAZON")) {
                                        ...
                                        try {
                                            ...
                                            final String[] procesos2 = new String[procesos.length];
